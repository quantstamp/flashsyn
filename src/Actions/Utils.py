...
def singleCollect(Actions, actionWrapper, para_product):
    ...
    for pp in para_product:
        if pp not in used:
            used.append(pp)
            forge.addDataCollector(pp, getattr(actionWrapper, "profitTokens", None))  # attack -> profit readout names
...
def testRealProfit_Batch(action_lists, para_lists, ActionWrapper):
    ...
    for ii in range(len(action_lists)):
        ...
        for jj in range(len(para_lists[ii])):
            ...
            index = forgeBatch.addDataCollector(para_list, getattr(ActionWrapper, "profitTokens", None))  # attack -> profit readout names
